...
function selectOption(event) {
  ...
  options.forEach(option => {
    option.disabled = true; // Desactiva los botones de opción para evitar selecciones adicionales
...
function showScore() {
  ...
  const message = `You answered ${score} questions how an introverted person would, so you are (${percentage.toFixed(2)}% introverted)`;
